...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        this.db.collection('item').aggregate([{$group:{_id:null,count:{$sum:1}}}]).toArray(function(err, result) {
             assert.equal(err, null);
             var totalNum=result[0].count;
             var category = {
                 _id: "All",
                 num: totalNum
             };
      ...
      this.db.collection('item').aggregate([{$group:{_id:"$category",num:{$sum:1}}},{$sort:{"_id":1}}]).toArray(function(err, result) {
        assert.equal(err, null);
    //    console.log(result);
    ...
    //    }
        ...
        //   reviews: 1
        // };
        var startLocation = page*itemsPerPage;
        var query;
        if(category == 'All'){
           query ={};
        }else{
    ...
    this.getNumItems = function(category, callback) {
         ...
         */
         ...
         if(category=="All")
            ...
            agg = [{$match:{"category":category}},{$group:{_id:"$category",count:{$sum:1}}}];

         this.db.collection('item').aggregate(agg).toArray(function(err, result) {
              assert.equal(err, null);
          //    console.log("tessssss"+result[0]);
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
        ...
        var startLocation = page*itemsPerPage;
        var queryString = {$text:{$search: query }};

       var cursor =  this.db.collection("item").find(queryString);
       cursor.sort({_id:1});
       cursor.skip(startLocation);
       cursor.limit(itemsPerPage);
       cursor.forEach(
    ...
    this.getNumSearchItems = function(query, callback) {
        ...
        */
        var agg =[{ $match: { $text: { $search: query} } },{$group:{_id:null ,count:{$sum:1}}}];
